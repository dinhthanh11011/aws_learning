 ...
 */
export function levelFromXp(xp: number): { level: number; into: number; span: number; nextAt: number } {
  let level = 1
  let need = 100
  let acc = 0
  while (xp >= acc + need) {
...
export function touchStreak(profile: Profile, now = new Date()): StreakUpdate {
  ...
  if (!profile.lastActiveDay) {
    const next = { ...profile, streak: 1, bestStreak: Math.max(1, profile.bestStreak), lastActiveDay: day }
...
export const ACHIEVEMENTS: AchievementDef[] = [
  { id: 'first-blood', title: 'First Answer', description: 'Answered your first exam-format question.', hint: 'Answer one question.', icon: '🎯' },
  { id: 'budget-alarm', title: 'Budget Alarm', description: 'Read every idle-cost warning before touching a real account.', hint: 'Review the teardown checklist.', icon: '💸' },
  { id: 'week-one', title: 'Seven Days', description: 'A seven-day streak. The habit is forming.', hint: 'Study seven days in a row.', icon: '🔥' },
  { id: 'month-one', title: 'Thirty Days', description: 'A thirty-day streak. This is how people actually pass.', hint: 'Study thirty days in a row.', icon: '🏔️' },
  { id: 'policy-master', title: 'Explicit Deny', description: 'Solved every IAM policy puzzle.', hint: 'Complete the IAM lab.', icon: '🔒' },
  { id: 'packet-whisperer', title: 'Packet Whisperer', description: 'Diagnosed every break-it challenge in the VPC lab.', hint: 'Break the VPC seven ways and predict each symptom.', icon: '📡' },
  { id: 'trigger-happy', title: 'Keyword Decoder', description: 'Drilled all the exam trigger phrases to mastery.', hint: 'Reach mastery on the keyword decoder.', icon: '🔑' },
  { id: 'first-exam', title: 'Full Paper', description: 'Sat a complete full-length timed exam.', hint: 'Complete a full exam simulation.', icon: '📝' },
  { id: 'passing-grade', title: 'Above the Line', description: 'Scored at or above the pass mark on a full paper.', hint: 'Pass a full exam simulation.', icon: '✅' },
  { id: 'eighty-club', title: 'Eighty Club', description: 'Scored 80%+ on a paper you had never seen. This is the signal to book.', hint: 'Score 80% on a fresh paper.', icon: '🎓' },
  { id: 'honest-review', title: 'Honest Reviewer', description: 'Logged twenty mistakes in your own words.', hint: 'Write up twenty mistakes.', icon: '📓' },
  { id: 'clean-log', title: 'Clean Log', description: 'Resolved every entry in your mistake log.', hint: 'Clear the mistake log.', icon: '🧹' },
  { id: 'tier-one', title: 'Core Fifty', description: 'Reached three rings or better on every core service.', hint: 'Master all tier-1 services.', icon: '⭐' },
  { id: 'the-whole-map', title: 'Cartographer', description: 'Visited every service card at least once.', hint: 'Open every service in the atlas.', icon: '🗺️' },
]
