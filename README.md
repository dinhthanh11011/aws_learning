...
| **Lessons** | One idea taught in the order that makes it stick: a picture before the definition, a real rule block read out line by line, the wrong answer written out before it is rejected. Seven so far, in two clusters: reachability — routing, the two filters, and why something cannot reach the internet — and identity — how IAM decides, roles rather than access keys, and envelope encryption. Each has something you step through and a recall check at the end of every idea. |
